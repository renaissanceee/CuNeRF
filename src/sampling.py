...
def cube_sampling(batch, depths, n_samples, is_train, R):
    ...
    steps = int(math.pow(n_samples, 1./3) + 1)# 64->4^3, step=5
    # (steps, steps, steps)
    ...
    t_vals = t_vals[1:, 1:, 1:].contiguous().view(-1, 3) # exclude first sample

    x_l, x_r = left.expand([n_cnts, n_samples]), right.expand([n_cnts, n_samples]) # [1024,1]->[1024,64] expand to build uniform samples
